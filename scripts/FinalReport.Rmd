...
# model = output.path("results/MR_ADbidir/", '(?<=/)[:graph:]+(?=/)')
...
# output.path = snakemake@params[["DataOutput"]] 

```

```{r sandbox, eval=T, echo=FALSE}
# model <- "MR_ADphenome"
model <- "MR_ADbidir"
...
```{r write-harmonized-data, echo=FALSE}
...
```{r, write-wide-MR-results, echo=FALSE}
mrresults.methods_presso %>% 
  select(-outcome, -exposure, -ends_with("Signif")) %>%
  ...
  write_csv(., glue("results/{model}/All/{model}_WideMRResults_{DATE}.csv"))
```
...
```{r, write-publication-results, echo=FALSE}
