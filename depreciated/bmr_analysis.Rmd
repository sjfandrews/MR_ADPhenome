...
params:
  trait_load.path: ~/Dropbox/Research/PostDoc-MSSM/2_MR/2_DerivedData/ldl/load_ldl_5e-6_BMRdat.csv
  trait_aaos.path: ~/Dropbox/Research/PostDoc-MSSM/2_MR/2_DerivedData/ldl/aaos_ldl_5e-6_BMRdat.csv
  trait_ab42.path: ~/Dropbox/Research/PostDoc-MSSM/2_MR/2_DerivedData/ldl/ab42_ldl_5e-6_BMRdat.csv
  trait_ptau.path: ~/Dropbox/Research/PostDoc-MSSM/2_MR/2_DerivedData/ldl/ptau_ldl_5e-6_BMRdat.csv
  trait_tau.path: ~/Dropbox/Research/PostDoc-MSSM/2_MR/2_DerivedData/ldl/tau_ldl_5e-6_BMRdat.csv
  trait_hipv.path: ~/Dropbox/Research/PostDoc-MSSM/2_MR/2_DerivedData/ldl/hipv_ldl_5e-6_BMRdat.csv
  out.path: 2_DerivedData/test/
  trait.name: ldl
  trait.code: ldl
  p.threshold: 5e-6
---
...
nsnps <- max(nrow(trait_LOAD.MRdat), nrow(trait_AAOS.MRdat), nrow(trait_hipv.MRdat))
...
**HV ~ `r params$trait.name`:** The MR-PRESSO global test for pleiotropy was `r if(hipv_presso < 0.05){paste('significant', ' (p = ', hipv_presso, ')', sep = "")} else {paste('non-significant')}`. `r if(hipv_presso < 0.05 & any(ls() %in% "hipv_snps")){paste('The following SNPs were removed due to pleiotropy:', paste(hipv_snps, collapse=", ") )}`
<br>
...
Figure 4 illustrates the SNP-specific associations with hipv versus the association between each SNP and `r params$trait.name`.

```{r scatter_plot hipv, echo=F, message = FALSE, warning=F, fig.width=7.5, fig.height=4.7, fig.align='center', fig.cap= 'Fig. 4: Scatterplot of SNP effects for the association of Trait and hipv'}

##  Scatter Plot
scatter.trait_hipv <- mr_scatter_plot(res.trait_hipv, trait_hipv.MRdat)
scatter.trait_hipv[[1]] + theme_bw() +
  theme(legend.position = "bottom", text = element_text(family="Times", size=12)) + scale_colour_discrete() +
  labs(y = paste0('Per allele change in \n', params$trait.name),
       x = 'Per allele change in risk of \n HV')
...
Figure 5 and Table 4 shows the SNP-specific effects and overall IVW, weighted median and Egger regression causal estimates of genetically predicted HV on `r params$trait.name`.

```{r forrest_plot hipv, echo=F, message = FALSE, warning=F, fig.width=7.5, fig.height=4.7, fig.align='center', fig.cap="Fig. 5: Forrest plot of Wald ratios and 95% CIs for SNP-specific and overall IVW, Weighted median and Egger associations"}

##  Forest Plot
forrest.trait_hipv <- mr_forest_plot(single.trait_hipv)
forrest.trait_hipv[[1]] + theme_classic() +
  theme(legend.position = "none", text = element_text(family="Times", size=12)) +
  labs(x = paste0('MR effect size of \nHV on ', params$trait.name))

```
<br>
  
  **Table 4:** MR estimates for HV and `r params$trait.name`
```{r MR hipv table, echo=F}
as.data.frame(single.trait_hipv %>% select(SNP, b, se, p))
...
```{r funnel_plot hipv, echo=F, message = FALSE, warning=F, fig.width=7.5, fig.height=4.7, fig.align='center', fig.cap='Fig. 6:  Funnel plot of the traitohol Conumption – hipv causal estimates against their precession'}
funnel.trait_hipv <- mr_funnel_plot(single.trait_hipv)
funnel.trait_hipv[[1]] + theme_bw() + theme(legend.position = "bottom", text = element_text(family="Times", size=12)) + scale_colour_discrete()

```
<br>
  
  **Table 5:** Heterogenity tests for HV and `r params$trait.name`
```{r heterogeneity consumption_hipv, echo=F}
...
**Table 6:** Test for directional pleitropy for HV and `r params$trait.name`
...
```{r heterogeneity trait_AB42, echo=F, eval=params$p.threshold == '5e-6'}
trait_ab.het <- mr_heterogeneity(trait_AB42.MRdat, method_list=c("mr_egger_regression", "mr_ivw"))

as.data.frame(trait_ab.het)
